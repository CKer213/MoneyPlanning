...
import { balanceCalc, addition, validate } from './App.ts';
...
function App() {
  ...
  const [expandPie, setExpandPie] = React.useState(false);
  ...
  const combinedData = list.reduce((acc, curr) => {
  ...
  function addTransaction () {
    let titleCheck = validate(transaction.title);
    let categoryCheck = validate(transaction.category);
    if (titleCheck && categoryCheck) {
      setExpandForm(false); 
      setList([...list, transaction]);
      setTotalIncome(addition(totalIncome, transaction.income));
      setTotalOutcome(addition(totalOutcome, transaction.outcome));
      setValidation(true);
      ...
      <div className="dashboard">
        <table className="dashboard-table">
          ...
          </tr>
          {
            list.map((eachList)=> {
              return(
                <tr className="dashboard-table-content">
        ...
        <div className="balance">
          ...
          <div className="balance-amount">{balanceCalc(budgetAmount,totalIncome,totalOutcome)}</div>
        </div>
      </div>
      <div className="feature">
        <TextField label="Budget" variant="standard" type="number" onChange={((e)=> setBudgetAmount(e.target.value))}/>
        ...
        <div className="feature-content">
          {
            expandForm === false ? 
            <Button variant="contained" onClick={()=>{setExpandForm(true); setTransaction({title:"",category:"",income:0,outcome:0})}}>Add Transaction</Button>
            :
        ...
        <div>{console.log(combinedData)}</div>
      </div>
    </div>
  );
}
